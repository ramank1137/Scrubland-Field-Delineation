...
def get_tiles_from_features(features, roi):
    candidates = ee.FeatureCollection(features).filterBounds(roi)
    def tag_inside(f):
        return f.set('inside', roi.geometry().contains(f.geometry(), 1))  # 1 m maxError
    
    tagged = candidates.map(tag_inside)
    tiles = tagged.filter(ee.Filter.eq('inside', True))
    return tiles

def get_chunks(features, chunk_size=20000):
...
def get_tiles(roi):
    ...
    if len(features) < 20000:
        return get_tiles_from_features(features, roi), points
    else:
        features_chunks = get_chunks(features)
        tiles = []
        for features in features_chunks:
            tiles.append(get_tiles_from_features(features, roi))
        return tiles, points
...
def write_to_drive(fc, folder_name, file_name):
    task = ee.batch.Export.table.toDrive(
        ...
        description='Working on ' + file_name,
        folder=folder_name,
...
def get_histogram(roi, tiles_path):
    ...
    lulc = ee.Image("projects/corestack-datasets/assets/datasets/LULC_v3_river_basin/pan_india_lulc_v3_2023_2024").clip(roi)
    ...
    clusters = emb.cluster(clusterer).clip(roi)
    ...
    tiles_with_hist = clusters.reduceRegions(
        ...
        crs=None,
        crsTransform=None,
        tileScale=4  # Increase tileScale if you still hit memory issues
    )
    return tiles_with_hist

def chunk_feature_collection(fc, chunk_size):
    """Splits a FeatureCollection into a list of FeatureCollections of size chunk_size."""
    size = fc.size().getInfo()
    chunks = []
    for i in range(0, size, chunk_size):
        chunk = fc.toList(chunk_size, i)
        chunk_fc = ee.FeatureCollection(chunk)
        chunks.append(chunk_fc)
    return chunks

def get_histogram_chunks(roi, tiles_path, chunk_size=100):
    start_date, end_date = get_prev_and_curr_year_dates()
    tiles = ee.FeatureCollection(tiles_path)
    # Split tiles into chunks
    tile_chunks = chunk_feature_collection(tiles, chunk_size)
    lulc = ee.Image("projects/corestack-datasets/assets/datasets/LULC_v3_river_basin/pan_india_lulc_v3_2023_2024").clip(roi)
    green = lulc.gte(6).And(lulc.lte(12))
    emb = ee.ImageCollection("GOOGLE/SATELLITE_EMBEDDING/V1/ANNUAL").filterDate(start_date, end_date).filterBounds(roi).mosaic().updateMask(green)
    ...
    # For each chunk, compute histogram
    tiles_with_hist_list = []
    for chunk_fc in tile_chunks:
        tiles_with_hist = clusters.reduceRegions(
            collection=chunk_fc,
            reducer=ee.Reducer.frequencyHistogram(),
            scale=10,
        ...
        tiles_with_hist_list.append(tiles_with_hist)
    return tiles_with_hist_list
...
def get_representative_tiles(directory):
    ...
    p = round(len(df)*3/100) # taking 3 percent of tiles as sampling region
...
def pre_process(roi, directory):
    if os.path.exists(directory + "/status.csv"):
        return
    ...
    if isinstance(tiles, list):
        paths = []
        for ind, tiles_ in enumerate(tiles):
            t_p = tiles_path + "_" + str(ind)
            paths.append(t_p)
            write_to_gee(tiles_, t_p)
        tiles = ee.FeatureCollection([])
        for p in paths:
            try:
                check = ee.FeatureCollection(p).limit(1).size().getInfo()
                t = ee.FeatureCollection(p)
                tiles = tiles.merge(t)
            except:
                print(p)
                pass
    ...
    tiles_with_hist = get_histogram_chunks(roi, tiles_path)
    print("Saving histogram")
    for i, twh in enumerate(tiles_with_hist):
        write_to_drive(twh, hist_tile_drive_path, directory + "_tiles_with_hist_" + str(i))
