...
MAX_WORKERS = min(6, os.cpu_count() or 1)  # tune if RAM is tight
...
def sample_points_from_tiles_parallel(aez_no: int):
    ...
    df["tile_geom"] = df["points"].apply(lambda s: parse_points_to_tile(s, assume_lonlat=POINTS_ARE_LON_LAT))
    log(f"[AEZ {aez_no}] Preparing {len(df)} tile(s). Using {MAX_WORKERS} worker(s). Start method: {MP_CTX.get_start_method()}")
